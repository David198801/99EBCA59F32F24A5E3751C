...
const upstream_mobile = 'www.south-plus.net'
...
async function fetchAndApply(request) {
    ...
    if (blocked_region.includes(region)) {
        ...
        if (content_type != null && content_type.includes('text/html')) {
            resp_content = await replace_response_text(original_response_clone, upstream_domain, url_hostname);
        } else {
            resp_content = original_response_clone.body
        }
		
		if (original_response.headers.get('sec-fetch-dest') == "image") {
            resp_content = original_response.arrayBuffer();
        }
