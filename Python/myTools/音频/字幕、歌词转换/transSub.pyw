...
def lrc_timestamp_to_ms(line) -> int:
    """将 pylrc 行对象时间转成毫秒"""
    # pylrc LyricLine 使用 time 属性（秒, float），直接用 time * 1000 得到毫秒
    time_sec = getattr(line, "time", None)
    if time_sec is not None:
        return int(time_sec * 1000)
    # 兼容回退：尝试 minutes/seconds/milliseconds（均为复数）
    minutes = getattr(line, "minutes", 0)
    seconds = getattr(line, "seconds", 0)
    milliseconds = getattr(line, "milliseconds", 0)
...
def _read_lrc_content(lrc_path: str) -> str:
    """读取 LRC 文件内容，自动尝试多种编码"""
    # LRC 文件常见编码：UTF-8 (带/不带 BOM)、GBK、GB2312
    encodings_to_try = ["utf-8-sig", "utf-8", "gbk", "gb2312", "gb18030", "latin-1"]
    for enc in encodings_to_try:
        try:
            with open(lrc_path, "r", encoding=enc) as f:
                content = f.read()
            # 验证内容可被 pylrc 正常解析（至少包含有效行）
            if content.strip():
                return content
        except (UnicodeDecodeError, UnicodeError):
            continue
    # 最终兜底：强制使用 utf-8 读取（让异常自然抛出）
...
def convert_lrc_to_subs(lrc_path: str) -> pysubs2.SSAFile:
    """LRC 转 pysubs2 对象"""
    content = _read_lrc_content(lrc_path)
