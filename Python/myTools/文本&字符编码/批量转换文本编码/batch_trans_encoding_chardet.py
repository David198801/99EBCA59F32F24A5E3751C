...
directory_to_convert = r'E:\BaiduNetdiskDownload\笔趣阁全站网络小说大合集 31w本[txt](2)'
...
# 是否检查最后修改时间（只处理修改时间早于LAST_TIME的文件）
IS_CHECK_LAST_TIME = True
# 最后修改时间
LAST_TIME = datetime.strptime('2025-09-18 00:00:00', '%Y-%m-%d %H:%M:%S')
        ...
        for file in files:
            if file.lower().split(".")[-1] in exts:
                ...
                if os.path.getsize(file_path) == 0:
                    continue
                # 如果IS_CHECK_LAST_TIME为True且文件修改时间大于等于LAST_TIME则continue
                if IS_CHECK_LAST_TIME:
                    # 获取文件的最后修改时间（浮点数时间戳）
                    file_mtime = os.path.getmtime(file_path)
                    # 将时间戳转换为datetime对象以便比较
                    file_mtime_dt = datetime.fromtimestamp(file_mtime)
                    # 如果文件修改时间晚于或等于LAST_TIME，则跳过
                    if file_mtime_dt >= LAST_TIME:
                        continue
