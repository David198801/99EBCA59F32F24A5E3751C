...
for i in date_number:#遍历每日
    ...
    for j in range(len(filenames_data)):#遍历每一个气象站（文件）数据
        ...
        month_data = []
        for l in all_data:
            if filenames_data[j][0:11] + i[0:6] in i:
                month_data.append(i)
            ...
            def writeData(da_name, da_col):  # d定义写入函数
                if filenames_data[j][0:11] + i[0:6] + da_name in m:
                    if m[(21 + m_day):(26 + m_day)] == '-9999':  # 检查无效值
                        ...
                        sheet.write(j + 1, da_col, m[(21 + m_day):(26 + m_day)])
            # 写入数据
            writeData('TMAX', 7)
            writeData('TMIN', 8)
            writeData('TOBS', 9)
        ...
        for n in all_meta_data:  # 遍历每一行
            if filenames_data[j][0:11] in n:  # 查找当前气候站数据
                ...
                sheet.write(j + 1, 3, n[31:37])
                sheet.write(j + 1, 4, n[12:20])
                sheet.write(j + 1, 5, n[21:30])
                sheet.write(j + 1, 6, i)
